    ...
    function __construct() {
        ...
        if ( current_user_can( 'manage_options' ) ) {
            if ( check_ajax_referer( 'upt_nonce', 'nonce', false ) ) {
                add_action( 'wp_ajax_upt_save', array( $this, 'save_settings' ) );
                add_action( 'wp_ajax_upt_sync', array( $this, 'sync' ) );
                add_action( 'wp_ajax_upt_reset', array( $this, 'reset' ) );
                add_action( 'wp_ajax_upt_heartbeat', array( $this, 'heartbeat' ) );
            }
        }

        add_action( 'wp_ajax_nopriv_upt_resume_sync', array( $this, 'resume_sync' ) );
    }
